...
.. note::
   ...
   The block above is generated by running ``inspect_data -h`` during doc build.
   If you build docs locally, install the package first (e.g. ``pip install .``).
...
The underlying Python API is documented under :doc:`api_reference` (see ``dl_utils.inspect_data_utils``).
